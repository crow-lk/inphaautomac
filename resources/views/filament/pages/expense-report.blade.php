...
// Total calculations
$totalProcurements = $groups['procurement_total'];
$totalSupplies = $groups['supply_total'];
$grandTotal = $totalProcurements + $totalSupplies;
@endphp
        ...
        <table class="w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="cursor-pointer font-semibold bg-gray-100 dark:bg-gray-800"
                    onclick="toggleGroup('procurements-group')">
                    <td class="px-4 py-2">Procurements <i class="fas fa-chevron-down"></i></td>
                    <td class="px-4 py-2 text-right">{{ number_format($totalProcurements, 2) }}</td>
                </tr>

                {{-- PROCUREMENTS LIST (NO QUANTITY) --}}
                @foreach($groups['procurements'] as $record)
                <tr class="hidden procurements-group bg-white dark:bg-gray-800">
                ...
                {{-- SUPPLIES HEADER --}}
                <tr class="cursor-pointer font-semibold bg-gray-100 dark:bg-gray-800"
                    onclick="toggleGroup('supplies-group')">
                    <td class="px-4 py-2">Supplies <i class="fas fa-chevron-down"></i></td>
                    <td class="px-4 py-2 text-right">{{ number_format($totalSupplies, 2) }}</td>
                </tr>

                {{-- SUPPLIES LIST (SHOW QUANTITY) --}}
                @foreach($groups['supplies'] as $record)
                <tr class="hidden supplies-group bg-white dark:bg-gray-800">
                    <td class="px-6 py-2 text-sm">
